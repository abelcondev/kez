...
//   KEZ_SKIP_DOWNLOAD=1      skip entirely, exit 0 (wrapper will guide if run)
//   KEZ_INSTALL_DRY_RUN=1    print the resolved plan as JSON, no network, exit 0
//   KEZ_INSTALL_PLATFORM=…   override process.platform (linux|darwin|win32|android)
//   KEZ_INSTALL_ARCH=…       override process.arch (x64|arm64)
//   KEZ_REPO=owner/name      override the GitHub repo (default abelcondev/kez)
//   KEZ_GITHUB_BASE_URL=…    override the download host (default https://github.com)
...
const REPO = process.env.KEZ_REPO || 'abelcondev/kez';
const BASE = (process.env.KEZ_GITHUB_BASE_URL || 'https://github.com').replace(/\/+$/, '');
...
const ALLOW_INSECURE = process.env.KEZ_ALLOW_INSECURE_DOWNLOAD === '1';
...
if (process.env.KEZ_SKIP_DOWNLOAD === '1') {
  warnSkip('KEZ_SKIP_DOWNLOAD=1 set — skipping native binary download.');
}

const rawPlatform = process.env.KEZ_INSTALL_PLATFORM || process.platform;
const rawArch = process.env.KEZ_INSTALL_ARCH || process.arch;
...
if (process.env.KEZ_INSTALL_DRY_RUN === '1') {
  ...
  if (!ALLOW_INSECURE && !url.startsWith('https://')) {
    fail(
      `refusing insecure download origin for ${url}: only https:// is allowed ` +
        `(set KEZ_ALLOW_INSECURE_DOWNLOAD=1 to override for local testing).`,
    );
  }
  let response;
  try {

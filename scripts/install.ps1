[CmdletBinding()]
param(
  [string]$Version = $env:KEZ_VERSION,
  [string]$Repository = $(if ($env:KEZ_REPO) { $env:KEZ_REPO } else { "abelcondev/kez" }),
  [string]$InstallDir = $env:KEZ_INSTALL_DIR,
  [string]$GitHubApi = $(if ($env:KEZ_GITHUB_API) { $env:KEZ_GITHUB_API } else { "https://api.github.com" }),
  [string]$GitHubBaseUrl = $(if ($env:KEZ_GITHUB_BASE_URL) { $env:KEZ_GITHUB_BASE_URL } else { "https://github.com" })
)
...
if ([string]::IsNullOrWhiteSpace($InstallDir)) {
  $InstallDir = Join-Path $env:LOCALAPPDATA "kez\bin"
}
...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("kez-install-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
  ...
  Write-Host "Installing kez $tag for windows-$arch"
  ...
  # The lean release archive ships just the kez.exe binary. (The deep pipeline's
  # windows sandbox-helper exes are not produced here.)
  $requiredFiles = @(
    "kez.exe"
  )
  ...
  $targetPath = Join-Path $InstallDir "kez.exe"
  ...
  if (-not (Test-ZeroPathContainsDir -PathValue $userPath -Dir $InstallDir)) {
    try {
      ...
      Write-Host "Added $InstallDir to your user PATH. Restart your terminal to use 'kez'."
    } catch {
      Write-Warning "Could not update your user PATH automatically: $_"
      Write-Warning "Add $InstallDir to PATH manually to run kez from any directory."
    }
  }

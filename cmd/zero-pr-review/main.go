...
import (
	...
	"github.com/abelcondev/kez/internal/review"
)
...
Environment:
  KEZ_REVIEW_DIFF_CHECK      Outcome for diff hygiene
  KEZ_REVIEW_TEST            Outcome for tests
  KEZ_REVIEW_BUILD           Outcome for build
  KEZ_REVIEW_SMOKE           Outcome for smoke build
  KEZ_CHANGED_FILES          Newline-separated changed file paths
  KEZ_REVIEW_HEAD_SHA        Pull request head SHA
  KEZ_PR_NUMBER              Pull request number
`)
	return err
}

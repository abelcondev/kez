...
import (
	...
	"github.com/abelcondev/kez/internal/perfbench"
)
...
func parseArgs(args []string, getenv func(string) string) (cliOptions, error) {
	iterations, err := readPositiveIntegerEnv(getenv, "KEZ_PERF_ITERATIONS", defaultIterations)
	if err != nil {
		return cliOptions{}, err
	}
	warmupIterations, err := readNonNegativeIntegerEnv(getenv, "KEZ_PERF_WARMUP_ITERATIONS", defaultWarmupIterations)
	if err != nil {
		return cliOptions{}, err
	}
	coldStartP95Ms, err := readPositiveNumberEnv(getenv, "KEZ_PERF_COLD_START_WARN_MS", perfbench.DefaultThresholds.ColdStartP95Ms)
	if err != nil {
		return cliOptions{}, err
	}
	firstOutputP95Ms, err := readPositiveNumberEnv(getenv, "KEZ_PERF_FIRST_OUTPUT_WARN_MS", perfbench.DefaultThresholds.FirstOutputP95Ms)
	if err != nil {
		return cliOptions{}, err
	}
	harnessEndRssMaxMb, err := readPositiveNumberEnv(getenv, "KEZ_PERF_HARNESS_END_RSS_WARN_MB", perfbench.DefaultThresholds.HarnessEndRssMaxMb)
	if err != nil {
		return cliOptions{}, err
	}
	options := cliOptions{
...
func helpText() string {
	return strings.Join([]string{
		...
		"  KEZ_PERF_ITERATIONS, KEZ_PERF_WARMUP_ITERATIONS",
		"  KEZ_PERF_COLD_START_WARN_MS, KEZ_PERF_FIRST_OUTPUT_WARN_MS",
		"  KEZ_PERF_HARNESS_END_RSS_WARN_MB",
	}, "\n") + "\n"
}

...
func TestParseArgsUsesCliAndEnvOverrides(t *testing.T) {
	env := func(key string) string {
		values := map[string]string{
			"KEZ_PERF_COLD_START_WARN_MS":      "250",
			"KEZ_PERF_HARNESS_END_RSS_WARN_MB": "384",
		}
		return values[key]
	}
...
func TestParseArgsUsesEnvOnlyThreshold(t *testing.T) {
	options, err := parseArgs(nil, func(key string) string {
		if key == "KEZ_PERF_FIRST_OUTPUT_WARN_MS" {
			return "610"
		}
		return ""
	})
...
func TestParseArgsRejectsInvalidValues(t *testing.T) {
	...
	if _, err := parseArgs(nil, func(key string) string {
		if key == "KEZ_PERF_ITERATIONS" {
			return "nope"
		}
		return ""
	}); err == nil || !strings.Contains(err.Error(), "KEZ_PERF_ITERATIONS must be a positive integer") {
...
func TestHelpText(t *testing.T) {
	...
	for _, want := range []string{"Usage: zero-perf-bench", "--iterations", "KEZ_PERF_ITERATIONS"} {

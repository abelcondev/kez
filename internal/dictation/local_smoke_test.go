...
//
//	KEZ_STT_TEST_BINARY=/path/to/sherpa-onnx-offline \
//	KEZ_STT_TEST_MODEL=/path/to/model-dir \
//	KEZ_STT_TEST_WAV=/path/to/16k-mono.wav \
//	go test ./internal/dictation/ -run TestLocalTranscribeRealBinary -v
func TestLocalTranscribeRealBinary(t *testing.T) {
	binary := os.Getenv("KEZ_STT_TEST_BINARY")
	model := os.Getenv("KEZ_STT_TEST_MODEL")
	wav := os.Getenv("KEZ_STT_TEST_WAV")
	if binary == "" || model == "" || wav == "" {
		t.Skip("set KEZ_STT_TEST_BINARY, KEZ_STT_TEST_MODEL, KEZ_STT_TEST_WAV to run the real-engine smoke test")
	}

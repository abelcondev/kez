...
import (
	...
	"github.com/abelcondev/kez/internal/providers/providerio"
	...
	"github.com/abelcondev/kez/internal/usercommands"
...
type model struct {
	...
	cwd                         string
	appVersion                  string
	userCommands                []usercommands.Command // file-sourced /commands (.kez/commands)
	...
	fadeActive         bool
	fadeDisabled       bool // streaming fade off (KEZ_NO_FADE / SSH / tmux / low-color / reduced motion)
	...
	// (KEZ_NO_STREAM_CLEAR=1). lastStreamClear rate-limits the redraws the
	...
	reducedMotion       bool // KEZ_REDUCED_MOTION / no-TTY: static spinner glyph, no fade
...
func newModel(ctx context.Context, options Options) model {
	...
	// Opt-in webhook fan-out (KEZ_NOTIFY_WEBHOOK_URL). Delivery failures stay
	...
	m := model{
		...
		themeMode:                   resolveThemeMode(options.Theme, os.Getenv("KEZ_THEME"), options.SavedTheme),
		hasDarkBg:                   true,
	...
	if v := strings.TrimSpace(os.Getenv("KEZ_NO_STREAM_CLEAR")); v != "" && v != "0" && !strings.EqualFold(v, "false") {
...
func (m model) updateModel(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case agentTextMsg:
		...
		// KEZ_NO_STREAM_CLEAR=1. A newline that arrives inside the throttle
		...
		// When the fade is disabled (KEZ_NO_FADE / SSH / tmux / low-color),
...
func (m model) dispatchCommand(command parsedCommand) (tea.Model, tea.Cmd) {
	...
	case commandSelfCorrect:
		...
		return m, nil
	case commandTurns:
		// Changing the budget mid-run would mutate the inherited KEZ_MAX_TURNS env
	...
	case commandProfile:
		...
		// turn budget (and its KEZ_MAX_TURNS propagation), so a change needs
	...
	case commandUnknown:
		...
		// from .kez/commands/<name>.md — expand its template and run it as a

...
import (
	...
	"github.com/abelcondev/kez/internal/browser"
...
// without requiring the user to export KEZ_OAUTH_ALLOW_PRESETS themselves.
...
func (m model) applyProviderWizard() (model, tea.Cmd) {
	...
	// and the KEZ_PROVIDER export (which pins spawned children) still point at
	...
	// this, a KEZ_PROVIDER exported by an earlier switch stays pointing at the
...
func providerWizardOAuthErrHint(provider providercatalog.Descriptor) string {
	if strings.EqualFold(provider.ID, "huggingface") {
		return "Hugging Face needs your own app: create one at " +
			"https://huggingface.co/settings/applications/new, then set KEZ_OAUTH_HUGGINGFACE_CLIENT_ID."
	}
	return ""
}

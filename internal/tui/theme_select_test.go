...
// resolveThemeMode precedence: explicit flag > KEZ_THEME env > auto.
...
func TestApplyThemeResolution(t *testing.T) {
	...
	cases := []struct {
		...
		{themeAuto, true, autoDarkTheme, darkPalette.ink},    // auto+dark -> dark (Claude)
		{themeAuto, false, autoLightTheme, lightPalette.ink}, // auto+light -> light (Claude)
...
func TestNewThemePresetsWired(t *testing.T) {
	...
	for _, name := range []string{"neon", "dune"} {
		if !validThemeMode(name) {
			t.Errorf("%q should be a valid --theme/KEZ_THEME value", name)
		}
	}
...
// The --theme flag and KEZ_THEME both resolve through resolveThemeMode, and

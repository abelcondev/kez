...
// exports KEZ_PROVIDER, warms discovery). On success the manager closes and
...
// live session follows a rename so KEZ_PROVIDER and the status line never
...
func (m model) saveManagerEdit() (model, tea.Cmd) {
	...
	// is running on: the exported KEZ_PROVIDER must resolve for spawned children.

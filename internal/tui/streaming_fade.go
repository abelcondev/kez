...
// explicit accessibility/preference switch via KEZ_REDUCED_MOTION, and is also
...
// whereas KEZ_NO_FADE only governs the streaming-text fade.
func reducedMotionEnabled(env func(string) string, profile colorprofile.Profile) bool {
	if v := strings.TrimSpace(env("KEZ_REDUCED_MOTION")); v != "" && v != "0" && !strings.EqualFold(v, "false") {
		return true
	}

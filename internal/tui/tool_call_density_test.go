...
func TestDisplayPathAbbreviatesHome(t *testing.T) {
	...
	abs := filepath.Join(home, "projects", "kez", "main.go")
	if got := displayPath(t.TempDir(), abs); got != "~/projects/kez/main.go" {
		t.Fatalf("displayPath under home = %q, want ~/projects/kez/main.go", got)
	}
}

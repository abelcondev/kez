...
func TestProviderWizardPersistsPastedKeyToUserConfig(t *testing.T) {
	...
	t.Setenv("KEZ_CRED_STORAGE", "encrypted-file")
...
// KEZ_PROVIDER exactly like the /model and /provider switch paths — a stale
...
// status line and the KEZ_PROVIDER export (which pins spawned children) still
...
func TestApplyProviderWizardPersistFailureLeavesLiveStateUnchanged(t *testing.T) {
	t.Setenv("KEZ_CRED_STORAGE", "encrypted-file") // never touch the real OS keychain: apiKey is secured before the persist fails

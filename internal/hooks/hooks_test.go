...
func TestResolvePathsUsesXDGLocations(t *testing.T) {
	...
	if paths.UserConfigPath != filepath.Join(dir, "config", "kez", "hooks.json") {
		t.Fatalf("user path = %q", paths.UserConfigPath)
	}
	if paths.ProjectConfigPath != filepath.Join(dir, ".kez", "hooks.json") {
		t.Fatalf("project path = %q", paths.ProjectConfigPath)
	}
	if paths.AuditPath != filepath.Join(dir, "data", "kez", "hooks", "audit.jsonl") {

...
type LoadOptions struct {
	...
	// ExcludeProject drops the project layer (./.kez/hooks.json) so only the
...
func ResolvePaths(options ResolvePathOptions) (Paths, error) {
	...
	return Paths{
		UserConfigPath:    filepath.Join(configHome, "kez", "hooks.json"),
		ProjectConfigPath: filepath.Join(cwd, ".kez", "hooks.json"),
		AuditPath:         filepath.Join(dataHome, "kez", "hooks", "audit.jsonl"),
	}, nil
}

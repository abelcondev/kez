...
import (
	...
	"github.com/abelcondev/kez/internal/aimlapi"
...
func ResolveMCP(options ResolveOptions) (MCPConfig, error) {
	...
	// ./.kez/config.json cannot register (and spawn) MCP servers. Fail-closed:
...
func mergeConfig(dst *FileConfig, src FileConfig) {
	...
	if strings.TrimSpace(src.Preferences.PermissionMode) != "" {
		dst.Preferences.PermissionMode = strings.TrimSpace(src.Preferences.PermissionMode)
	}
...
func mergeProjectConfig(dst *FileConfig, src FileConfig) error {
	...
	// repo's .kez/config.json must not be able to disable the sandbox that
	...
	// config: a cloned repo's .kez/config.json must not be able to grant
...
const ActiveProviderEnv = "KEZ_PROVIDER"
...
const MaxTurnsEnv = "KEZ_MAX_TURNS"

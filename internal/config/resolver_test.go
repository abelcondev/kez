...
func TestResolveLoadsProviderCatalogSnakeAndCamelJSONFields(t *testing.T) {
	path := writeConfig(t, `{
		...
		"providers": [{
			...
			"api_key_env": "KEZ_SNAKE_API_KEY",
			"api_format": "responses",
			"auth_header": "X-API-Key",
			"auth_scheme": "Token",
			"auth_header_value": "env:KEZ_SNAKE_HEADER"
		}, {
			"name": "camel",
			"provider": "anthropic",
			"model": "camel-model",
			"catalogID": "anthropic",
			"apiKeyEnv": "KEZ_CAMEL_API_KEY",
			"apiFormat": "messages",
			"authHeader": "Authorization",
			"authScheme": "Bearer",
			"authHeaderValue": "env:KEZ_CAMEL_HEADER"
		}]
	}`)
	...
	if resolved.Provider.APIKeyEnv != "KEZ_SNAKE_API_KEY" {
	...
	if resolved.Provider.AuthHeaderValue != "env:KEZ_SNAKE_HEADER" {
	...
	if camel.APIKeyEnv != "KEZ_CAMEL_API_KEY" {
	...
	if camel.AuthHeaderValue != "env:KEZ_CAMEL_HEADER" {
...
func TestResolveMergesProviderCatalogFieldsByLayerPrecedence(t *testing.T) {
	...
	projectPath := writeConfig(t, `{
		"providers": [{
			...
			"authHeaderValue": "env:KEZ_PROJECT_HEADER"
		}]
	}`)
	...
	if resolved.Provider.APIKeyEnv != "KEZ_USER_API_KEY" {
	...
	if resolved.Provider.AuthHeaderValue != "env:KEZ_PROJECT_HEADER" {
...
func TestResolveAPIKeyEnvLooksUpEnvOnlyWhenAPIKeyMissing(t *testing.T) {
	path := writeConfig(t, `{
		...
		"providers": [{
			"name": "from-env",
			"provider": "openai",
			"apiKeyEnv": "KEZ_FROM_ENV_API_KEY",
			"model": "gpt-from-env"
		}, {
			"name": "direct",
			"provider": "openai",
			"apiKey": "sk-direct",
			"apiKeyEnv": "KEZ_DIRECT_API_KEY",
			"model": "gpt-direct"
		}]
	}`)
	...
	resolved, err := Resolve(ResolveOptions{
		...
		Env: map[string]string{
			"KEZ_FROM_ENV_API_KEY": "sk-from-env",
			"KEZ_DIRECT_API_KEY":   "sk-should-not-win",
		},
	})
	if err != nil {
		t.Fatalf("Resolve() error = %v", err)
	...
	if resolved.Provider.APIKeyEnv != "KEZ_FROM_ENV_API_KEY" {
...
func TestResolveAPIKeyEnvRedactsResolvedSecretOnErrors(t *testing.T) {
	...
	_, err := Resolve(ResolveOptions{
		ProjectConfigPath: path,
		Env: map[string]string{
			"KEZ_CUSTOM_API_KEY": "sk-env-secret-value",
		},
	})
	if err == nil {
...
func TestResolveReplacesMCPServerOverlayCollections(t *testing.T) {
	...
	if _, ok := docs.Env["KEZ_DOCS_TOKEN"]; ok || docs.Env["KEZ_DOCS_PROJECT"] != "1" {
		t.Fatalf("docs.Env = %#v, want KEZ_DOCS_TOKEN absent and KEZ_DOCS_PROJECT=1", docs.Env)
	}

...
func TestDefaultResolveOptionsUsesExistingConfigPathsAndProviderCommand(t *testing.T) {
	...
	userPath := filepath.Join(userConfigRoot, "kez", "config.json")
	projectPath := filepath.Join(workspaceRoot, ".kez", "config.json")
	writeFileAt(t, userPath, "{}")
	writeFileAt(t, projectPath, "{}")
	t.Setenv("KEZ_PROVIDER_COMMAND", "  zero-provider  ")
...
func TestDefaultResolveOptionsRejectsDirectoryConfigPaths(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			makeDirectory: func(t *testing.T, userConfigRoot string, workspaceRoot string) string {
				t.Helper()
				path := filepath.Join(userConfigRoot, "kez", "config.json")
				mkdirAll(t, path)
				return path
			},
		},
		{
			...
			makeDirectory: func(t *testing.T, userConfigRoot string, workspaceRoot string) string {
				t.Helper()
				path := filepath.Join(workspaceRoot, ".kez", "config.json")
				mkdirAll(t, path)
				return path
			},
		},
...
func TestDefaultUserConfigPathUsesXDGConfigOnMacOS(t *testing.T) {
	...
	want := filepath.Join(home, ".config", "kez", "config.json")
	if path != want {

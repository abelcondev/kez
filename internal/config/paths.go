...
func DefaultResolveOptions(workspaceRoot string) (ResolveOptions, error) {
	...
	projectConfigPath, err := existingConfigFile(filepath.Join(workspaceRoot, ".kez", "config.json"))
	if err != nil {
		return ResolveOptions{}, err
	}
	...
	return ResolveOptions{
		...
		ProviderCommand:   strings.TrimSpace(os.Getenv("KEZ_PROVIDER_COMMAND")),
	}, nil
}
...
func DefaultUserConfigPath() (string, error) {
	...
	return filepath.Join(userConfigDir, "kez", "config.json"), nil
}

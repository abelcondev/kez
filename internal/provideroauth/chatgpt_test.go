...
// preset, so the resolver can build a Config without KEZ_OAUTH_CHATGPT_* vars.
func chatgptTestEnv() map[string]string {
	return map[string]string{"KEZ_OAUTH_ALLOW_PRESETS": "1"}
}
...
func TestChatGPTLoginUsesPreset(t *testing.T) {
	...
	// overriding KEZ_OAUTH_CHATGPT_CLIENT_ID.
	env := chatgptTestEnv()
	env["KEZ_OAUTH_CHATGPT_AUTHORIZE_URL"] = ts.AuthorizeURL()
	env["KEZ_OAUTH_CHATGPT_TOKEN_URL"] = ts.TokenURL()
...
func TestChatGPTLoginMissingAuthorizationEndpointErrors(t *testing.T) {
	...
	env := chatgptTestEnv()
	env["KEZ_OAUTH_CHATGPT_AUTHORIZE_URL"] = ""
	env["KEZ_OAUTH_CHATGPT_TOKEN_URL"] = "https://chatgpt.invalid/oauth/token"

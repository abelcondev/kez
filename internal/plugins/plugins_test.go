...
func TestLoadDiscoversProjectPluginWhenNotExcluded(t *testing.T) {
	dir := t.TempDir()
	writePluginManifest(t, filepath.Join(dir, ".kez", "plugins", "demo"), map[string]any{
		"schemaVersion": 1,
		"id":            "zero.project",
		"name":          "Project Plugin",
		"version":       "0.1.0",
...
func TestLoadKeepsUserPluginWhenProjectExcluded(t *testing.T) {
	dir := t.TempDir()
	// User plugin lives under the resolved XDG config home; project plugin under ./.kez.
	writePluginManifest(t, filepath.Join(dir, "xdg", "kez", "plugins", "user"), map[string]any{
		"schemaVersion": 1,
		"id":            "zero.user",
		"name":          "User Plugin",
		"version":       "0.1.0",
...
func TestResolveRootsUsesConfigHomeAndProjectRoot(t *testing.T) {
	...
	if roots[0].Path != filepath.Join(dir, "xdg", "kez", "plugins") {
		t.Fatalf("user root = %q", roots[0].Path)
	}
	if roots[1].Path != filepath.Join(dir, ".kez", "plugins") {

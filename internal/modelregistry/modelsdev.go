...
// modelsDevCachePath returns the on-disk cache location. KEZ_MODELS_CACHE_PATH
// overrides it (used by tests and unusual setups).
func modelsDevCachePath() (string, error) {
	if override := strings.TrimSpace(os.Getenv("KEZ_MODELS_CACHE_PATH")); override != "" {
		return override, nil
	}
	...
	return filepath.Join(base, "kez", "modelsdev.json"), nil
}
...
// KEZ_DISABLE_MODELS_FETCH disables both the overlay and the fetch.
...
func cachedModelsDevProviders() map[string]map[string]modelsDevModel {
	if !modelsDevEnabled.Load() || strings.TrimSpace(os.Getenv("KEZ_DISABLE_MODELS_FETCH")) != "" {
		return nil
	}
...
// by KEZ_DISABLE_MODELS_FETCH. The URL can be overridden with KEZ_MODELS_URL.
func RefreshModelsDevCache(ctx context.Context) error {
	if strings.TrimSpace(os.Getenv("KEZ_DISABLE_MODELS_FETCH")) != "" {
		return nil
	}
	path, err := modelsDevCachePath()
	if err != nil {
	...
	url := strings.TrimSpace(os.Getenv("KEZ_MODELS_URL"))
	if url == "" {

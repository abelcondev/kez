...
func TestLinuxHelperRealSandboxSmoke(t *testing.T) {
	if os.Getenv("KEZ_SANDBOX_REAL_SMOKE") != "1" {
		t.Skip("set KEZ_SANDBOX_REAL_SMOKE=1 to run real sandbox smoke tests")
	}
...
func TestLinuxHelperPreservesAbsentProtectedMetadata(t *testing.T) {
	...
	command, plan, err := engine.CommandContext(ctx, CommandSpec{
		Name: "/bin/sh",
		Args: []string{"-c", "set -eu; test ! -e .kez; mkdir .kez"},
		Dir:  root,
	})
	if err != nil {
	...
	if _, err := os.Lstat(filepath.Join(root, ".kez")); !os.IsNotExist(err) {
	...
	if report.Denial.Capability.Scope != filepath.Join(root, ".kez") {

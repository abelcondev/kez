...
func TestBuildCommandPlanDegradedFallbackScrubsInheritedEnv(t *testing.T) {
	...
	t.Setenv("KEZ_OAUTH_ACME_CLIENT_SECRET", "oauth-secret")
	...
	for _, entry := range plan.Env {
		...
		if strings.EqualFold(key, "COMPANY_LLM_SECRET") || strings.EqualFold(key, "KEZ_OAUTH_ACME_CLIENT_SECRET") {
...
func TestSeatbeltProfileProtectsMetadataAndDenyOrdering(t *testing.T) {
	...
	for _, want := range []string{
		...
		`(deny file-write* (regex #"^/repo/\.kez(/.*)?$"))`,
		denySecretReadRule,
...
func TestScrubSensitiveEnv(t *testing.T) {
	inputEnv := []string{
		...
		"KEZ_OAUTH_MY_SVC_CLIENT_SECRET=oauth-secret",
		"kez_oauth_second_client_secret=case-insensitive-secret",
...
func TestEngineScrubsConfiguredSensitiveEnvKeys(t *testing.T) {
	...
	plan, err := engine.BuildCommandPlan(CommandSpec{
		...
		Env: []string{
			"PATH=/usr/bin",
			"COMPANY_LLM_SECRET=custom-secret",
			"KEZ_OAUTH_CUSTOM_CLIENT_SECRET=oauth-secret",
			"SAFE_VAR=hello",
		},
	})
	if err != nil {
	...
	for _, entry := range plan.Env {
		key, _, _ := strings.Cut(entry, "=")
		if strings.EqualFold(key, "COMPANY_LLM_SECRET") || strings.EqualFold(key, "KEZ_OAUTH_CUSTOM_CLIENT_SECRET") {

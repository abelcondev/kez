...
func TestNewRoutesChatGPTCatalogToCodexProvider(t *testing.T) {
	...
	t.Setenv("KEZ_OAUTH_TOKENS_PATH", t.TempDir()+"/tokens.json")
...
// returns a Store on it. Pinning KEZ_OAUTH_STORAGE matters as much as the
// path: an inherited "keyring" value would send NewStore to the OS keychain
// and ignore KEZ_OAUTH_TOKENS_PATH entirely, making the test read/write the

...
type Options struct {
	...
	// KEZ_STREAM_IDLE_TIMEOUT override or providerio.DefaultStreamIdleTimeout.
	...
	// enabled; the KEZ_DISABLE_PROMPT_CACHE_KEY env kill switch still applies
...
func (provider *Provider) openAIRequest(request zeroruntime.CompletionRequest) chatCompletionRequest {
	...
	// endpoint can still force-omit via KEZ_DISABLE_PROMPT_CACHE_KEY=1.
...
// promptCacheKeyDisabled reports whether the KEZ_DISABLE_PROMPT_CACHE_KEY
// kill switch is set to a truthy value. "0" and "false" (any case) are
// no-ops, matching how KEZ_FORMAT_ON_WRITE parses boolean flags, so an
// explicitly-disabled toggle never flips the behavior it names.
func promptCacheKeyDisabled() bool {
	value := strings.TrimSpace(os.Getenv("KEZ_DISABLE_PROMPT_CACHE_KEY"))

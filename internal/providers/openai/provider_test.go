...
func TestStreamCompletionEmitsTextUsageAndDone(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeSSE(w, `{"choices":[{"delta":{"content":"kez"}}],"usage":{"prompt_tokens":12,"completion_tokens":5,"prompt_tokens_details":{"cached_tokens":3}}}`)
		writeSSE(w, `[DONE]`)
	})

	events := collectProviderEvents(t, provider)
	assertEvent(t, events[0], zeroruntime.StreamEventText, "hello ")
	assertEvent(t, events[1], zeroruntime.StreamEventText, "kez")
...
// and KEZ_DISABLE_PROMPT_CACHE_KEY is the env kill switch for any endpoint.
func TestOpenAIRequestPromptCacheKey(t *testing.T) {
	...
	t.Setenv("KEZ_DISABLE_PROMPT_CACHE_KEY", "1")
	req = provider.openAIRequest(zeroruntime.CompletionRequest{
		Messages:       messages,
		PromptCacheKey: "sess_123",
	})
	...
	// truthy values flip the toggle (same parsing as KEZ_FORMAT_ON_WRITE).
	for _, value := range []string{"0", "false", "FALSE"} {
		t.Setenv("KEZ_DISABLE_PROMPT_CACHE_KEY", value)
		req = provider.openAIRequest(zeroruntime.CompletionRequest{
			Messages:       messages,
			PromptCacheKey: "sess_123",
		})
		if req.PromptCacheKey != "sess_123" {
			t.Fatalf("KEZ_DISABLE_PROMPT_CACHE_KEY=%q must be a no-op; PromptCacheKey = %q", value, req.PromptCacheKey)
		}
	}
}

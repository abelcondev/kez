...
// activeProviderEnvOverride returns the KEZ_PROVIDER value when it is set and
...
// env var is unset. applyEnv (resolver.go) makes KEZ_PROVIDER win over
...
func reportUnpersistedProviderUse(stdout, stderr io.Writer, deps appDeps, options providerUseOptions, configPath string) (int, bool) {
	...
	message := fmt.Sprintf(
		"Provider %q is not saved in config.json (likely set via an environment variable), so there is no saved profile to switch to.\nIt is available whenever its environment variable is set, but is only active when selected (for example via KEZ_PROVIDER); unset its environment variable to stop Zero from detecting it automatically.",
		options.name,
	)
	if options.json {
		if err := writePrettyJSON(stdout, map[string]any{

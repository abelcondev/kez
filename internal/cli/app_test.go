...
import (
	...
	"github.com/abelcondev/kez/internal/workspacetrust"
...
func TestRunPrintsVersion(t *testing.T) {
	...
	if got := stdout.String(); got != brand.Name+" dev\n" {
...
func TestRunVersionRedirectedFile(t *testing.T) {
	...
	if got := string(content); got != brand.Name+" dev\n" {
		t.Fatalf("expected version output, got %q", got)
	}
	if stderr.Len() != 0 {
		t.Fatalf("expected empty stderr, got %q", stderr.String())
...
func TestRunNoArgsLaunchesSetupTUIWithNilProviderWhenNoProviderConfigured(t *testing.T) {
	...
	projectConfigPath := filepath.Join(cwd, ".kez", "config.json")
	...
	userConfigPath := filepath.Join(t.TempDir(), "kez", "config.json")
...
// and the real ./.kez/config.json.
func TestRunInteractiveSurfacesMCPTrustNotice(t *testing.T) {
	setTrustConfigRoot(t)
	repo := t.TempDir()
	if err := os.MkdirAll(filepath.Join(repo, ".kez"), 0o700); err != nil {
		t.Fatal(err)
	}
	body := `{"mcp":{"servers":{"proj":{"type":"stdio","command":"proj-cmd"}}}}`
	if err := os.WriteFile(filepath.Join(repo, ".kez", "config.json"), []byte(body), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestRunUpdateCheckTextAndJSON(t *testing.T) {
	result := update.Result{
		CurrentVersion: "dev",
		LatestVersion:  "0.2.0",
		ReleaseURL:     "https://github.com/abelcondev/kez/releases/tag/v0.2.0",
		TagName:        "v0.2.0",
	...
	exitCode := runWithDeps([]string{"update", "--check"}, &stdout, &stderr, appDeps{
		checkUpdate: func(context.Context, update.Options) (update.Result, error) {
			return update.Result{
				CurrentVersion:  "dev",
				LatestVersion:   "dev",
				ReleaseURL:      "https://github.com/abelcondev/kez/releases/tag/dev",
				TagName:         "dev",
...
func TestRunThemeFlagPopulatesTUIOptions(t *testing.T) {
	...
	// over KEZ_THEME). Previously Options.Theme was read but never set by the CLI.
...
func TestRunNoArgsEntersSetupWhenActiveProviderMissesModel(t *testing.T) {
	...
	exitCode := runWithDeps([]string{}, &stdout, &stderr, appDeps{
		...
		userConfigPath: func() (string, error) { return filepath.Join(t.TempDir(), "kez", "config.json"), nil },
		registerMCPTools: func(context.Context, *tools.Registry, config.MCPConfig, mcp.RegisterOptions) (mcpToolRuntime, error) {
			return noopMCPRuntime{}, nil
		},
		runTUI: func(_ context.Context, options tui.Options) int {

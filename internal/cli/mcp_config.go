...
// projectMCPConfigExists reports whether the workspace's project ./.kez/config.json
...
func projectMCPConfigExists(workspaceRoot string) bool {
	...
	data, err := os.ReadFile(filepath.Join(workspaceRoot, ".kez", "config.json"))
	if err != nil {
		return false
	}
...
func writeMCPWritableConfig(path string, cfg mcpWritableConfig) error {
	...
	tmp, err := os.CreateTemp(dir, ".kez-config-*.tmp")
	if err != nil {

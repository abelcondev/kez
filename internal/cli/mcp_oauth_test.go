...
// ./.kez/config.json must be dropped before login. `zero mcp oauth login` must refuse
...
func TestRunMCPOAuthLoginGatedInUntrustedWorkspace(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(cwd, ".kez"), 0o700); err != nil {
		t.Fatal(err)
	}
	body := `{"mcp":{"servers":{"remote":{"type":"http","url":"` + oauthSrv.URL + `","auth":"oauth"}}}}`
	if err := os.WriteFile(filepath.Join(cwd, ".kez", "config.json"), []byte(body), 0o600); err != nil {
		t.Fatal(err)
	}
	store, err := mcp.NewTokenStore(mcp.TokenStoreOptions{
		FilePath: filepath.Join(t.TempDir(), "mcp-oauth-tokens.json"),

...
// writeProjectHooks writes a project ./.kez/hooks.json under repo with the given
// hooks and top-level enabled flag.
func writeProjectHooks(t *testing.T, repo string, enabled bool, defs ...hooks.Definition) {
	t.Helper()
	path := filepath.Join(repo, ".kez", "hooks.json")
...
func writeUserHooks(t *testing.T, configRoot string, enabled bool, defs ...hooks.Definition) {
	t.Helper()
	path := filepath.Join(configRoot, "kez", "hooks.json")
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
...
func TestPluginGateUntrustedExcludesProject(t *testing.T) {
	setTrustConfigRoot(t)
	repo := t.TempDir()
	// A ./.kez/plugins dir so the skip report flags an excluded project config.
	if err := os.MkdirAll(filepath.Join(repo, ".kez", "plugins"), 0o700); err != nil {
	...
	if !activation.excludedProjectConfig {
		t.Fatalf("skip report should flag the excluded ./.kez/plugins dir")
	}

...
// writeMarkerHook writes a ./.kez/hooks.json under dir whose enabled beforeTool
...
func writeMarkerHook(t *testing.T, dir, markerPath string) {
	...
	if err := os.MkdirAll(filepath.Join(dir, ".kez"), 0o755); err != nil {
		t.Fatal(err)
	}
	body := `{"enabled":true,"hooks":[{"id":"m","event":"beforeTool","command":"/bin/sh","args":["-c","touch '` + markerPath + `'"],"enabled":true}]}`
	if err := os.WriteFile(filepath.Join(dir, ".kez", "hooks.json"), []byte(body), 0o644); err != nil {
		t.Fatal(err)
	}
}
...
// trust verdict and the real ./.kez/config.json that projectMCPConfigExists reads.
...
// checkout carries the committed .kez/hooks.json; the gate must load it only when

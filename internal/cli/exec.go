...
import (
	...
	"github.com/abelcondev/kez/internal/notify"
	...
	"github.com/abelcondev/kez/internal/specmode"
	"github.com/abelcondev/kez/internal/streamjson"
	"github.com/abelcondev/kez/internal/tools"
	"github.com/abelcondev/kez/internal/trace"
	"github.com/abelcondev/kez/internal/usage"
...
type execOptions struct {
	...
	// the KEZ_TRACE env var when the flag is absent. Off by default: a run
...
func runExec(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if !config.HasProviderProfile(resolved.Provider) {
		return writeExecProviderError(stdout, stderr, options.outputFormat, "provider_error", "No provider configured. Run `zero setup` (guided), `zero auth` (OAuth providers), set a provider API key env var (e.g. OPENAI_API_KEY / ANTHROPIC_API_KEY / GEMINI_API_KEY), or add .kez/config.json.")
	}
	...
	// Optimized OpenAI turn sessions (KEZ_OPENAI_TURN_SESSION, default off).
	...
	// Opt-in webhook fan-out (KEZ_NOTIFY_WEBHOOK_URL). Headless runs can safely
	...
	// there is no per-turn trace to emit. Reject --trace / KEZ_TRACE up front with
	// a clear error rather than silently accepting and writing nothing.
	if options.useSpec && resolveTracePath(options) != "" {
		return writeExecFormatUsageError(stdout, stderr, options.outputFormat, "--trace / KEZ_TRACE are not supported for spec-draft runs")
	}
	if options.useSpec {
	...
	// Per-turn tracing is opt-in (--trace <path> or KEZ_TRACE=<path>). The
...
// value when set, else the KEZ_TRACE env var, else "" (tracing off). A value of
// "-" means "write to stderr".
func resolveTracePath(options execOptions) string {
	if v := strings.TrimSpace(options.tracePath); v != "" {
		return v
	}
	if v := strings.TrimSpace(os.Getenv("KEZ_TRACE")); v != "" {
		return v
	}
	return ""
}

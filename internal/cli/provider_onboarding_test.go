...
// The write to config.json still succeeds, but when KEZ_PROVIDER names a
...
func TestRunProvidersUseWarnsWhenEnvOverrides(t *testing.T) {
	...
	deps.getenv = func(key string) string {
		if key == config.ActiveProviderEnv {
			return "work" // KEZ_PROVIDER=work overrides the switch to fast
		}
		return ""
	}
...
// No override note when KEZ_PROVIDER is unset or already names the selection.

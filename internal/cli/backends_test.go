...
import (
	...
	"github.com/abelcondev/kez/internal/zerocommands"
)
...
func TestRunBackendsDoctorDoesNotConnectOrExecuteConfiguredBackends(t *testing.T) {
	...
	hookConfigPath := filepath.Join(cwd, ".kez", "hooks.json")
	...
	pluginDir := filepath.Join(cwd, ".kez", "plugins", "sentinel")
	...
	deps := appDeps{
		...
		loadPlugins: func(options plugins.LoadOptions) (plugins.LoadResult, error) {
			return plugins.Load(plugins.LoadOptions{
				Roots: []plugins.Root{{Source: plugins.SourceProject, Path: filepath.Join(cwd, ".kez", "plugins")}},
				Cwd:   options.Cwd,
			})
		},
		registerMCPTools: func(context.Context, *tools.Registry, config.MCPConfig, mcp.RegisterOptions) (mcpToolRuntime, error) {

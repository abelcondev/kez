...
func TestRunSpecialistListShowAndPath(t *testing.T) {
	...
	userDir := filepath.Join(configRoot, "kez", "specialists")
...
Find the likely failure area.`)
	...
	if !strings.Contains(stdout.String(), userDir) || !strings.Contains(stdout.String(), filepath.Join(cwd, ".kez", "specialists")) {
...
func TestRunSpecialistShowAndPathJSON(t *testing.T) {
	...
	if pathPayload.UserDir == "" || pathPayload.ProjectDir != filepath.Join(cwd, ".kez", "specialists") {
...
func TestRunSpecialistCreateProjectAndRejectDuplicate(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(cwd, ".kez", "specialists", "project-helper.md")); err != nil {

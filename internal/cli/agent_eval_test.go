...
import (
	...
	"github.com/abelcondev/kez/internal/agenteval"
)
...
func TestRunEvalBenchJSONModePassesHarnessOptions(t *testing.T) {
	...
	exitCode := runWithDeps([]string{
		...
		runAgentEval: func(ctx context.Context, options agentEvalOptions) (agentEvalReport, error) {
			...
			if got, want := strings.Join(options.AgentCommand, "\x00"), strings.Join([]string{"kez", "exec", "{prompt}"}, "\x00"); got != want {
...
func TestRunEvalRunRejectsBenchOnlyFlags(t *testing.T) {
	tests := []struct {
		...
		{
			name: "agent command",
			args: []string{"eval", "run", "--suite", "evals/context.json", "--agent-command", "kez", "exec", "{prompt}"},

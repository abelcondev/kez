...
import (
	...
	"github.com/abelcondev/kez/internal/agent"
	"github.com/abelcondev/kez/internal/brand"
	"github.com/abelcondev/kez/internal/config"
	"github.com/abelcondev/kez/internal/execution"
	"github.com/abelcondev/kez/internal/hooks"
	"github.com/abelcondev/kez/internal/localcontrol"
	"github.com/abelcondev/kez/internal/mcp"
	"github.com/abelcondev/kez/internal/modelregistry"
	"github.com/abelcondev/kez/internal/observability"
	"github.com/abelcondev/kez/internal/plugins"
	"github.com/abelcondev/kez/internal/providerhealth"
	"github.com/abelcondev/kez/internal/providermodeldiscovery"
	"github.com/abelcondev/kez/internal/provideroauth"
	"github.com/abelcondev/kez/internal/provideronboarding"
	"github.com/abelcondev/kez/internal/providers"
	...
	"github.com/abelcondev/kez/internal/sessions"
	"github.com/abelcondev/kez/internal/skills"
	"github.com/abelcondev/kez/internal/specialist"
	"github.com/abelcondev/kez/internal/swarm"
	"github.com/abelcondev/kez/internal/tools"
	"github.com/abelcondev/kez/internal/tui"
	"github.com/abelcondev/kez/internal/update"
	"github.com/abelcondev/kez/internal/verify"
	"github.com/abelcondev/kez/internal/worktrees"
	"github.com/abelcondev/kez/internal/zerogit"
...
type appDeps struct {
	...
	// against ambient vars like KEZ_PROVIDER unless they inject it). nil ⇒ empty.
...
func runWithDeps(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) (exitCode int) {
	...
	// KEZ_THEME). Re-split --add-dir afterward so it may appear on either side of --theme.
	...
	case "--skip-permissions-unsafe":
		...
		for _, arg := range rest {
			if strings.TrimSpace(arg) != "" {
				return writeAppError(stderr, "--skip-permissions-unsafe launches the interactive TUI and takes no prompt or subcommand; for a one-shot unsafe run use `"+brand.Name+" exec --skip-permissions-unsafe -p \"...\"`", 1)
			}
		}
	...
	case "-v", "--version", "version":
		for _, a := range args[1:] {
			if a == "-h" || a == "--help" {
				if _, err := fmt.Fprintln(stdout, "Usage: "+brand.Name+" version\n\nPrint the "+brand.DisplayName+" CLI version. Takes no flags."); err != nil {
					return 1
				}
				return 0
			}
		...
		if !stdoutIsTerminal(stdout) {
			if _, err := fmt.Fprintf(stdout, "%s %s\n", brand.Name, version); err != nil {
				return 1
			}
			return 0
		}
		if _, err := fmt.Fprintf(stdout, "%s\n\n%s %s\n", tui.Wordmark(), brand.Name, version); err != nil {
			return 1
		}
		return 0
	case "-p", "--prompt":
	...
	case "sdd":
		return runSDD(args[1:], stdout, stderr, deps)
	case "init":
...
func runInteractiveTUIWithSetup(stderr io.Writer, deps appDeps, permissionMode agent.PermissionMode, addDirs []string, theme string, forceSetup bool) int {
	...
	// untrusted workspace must not spawn its ./.kez/config.json stdio MCP servers.
...
func localArtifactsDirFromConfig(workspaceRoot string, cfg config.LocalControlConfig) string {
	...
	if dir == "" {
		dir = filepath.Join(".kez", "artifacts")
	}
...
func registerSpecialistTools(registry *tools.Registry, workspaceRoot string, maxTeamSize int) (*agentToolRuntime, error) {
	...
	sw, err := swarm.New(swarm.Options{
		BaseDir:     filepath.Join(workspaceRoot, ".kez", "swarm"),
...
func writeHelp(w io.Writer) error {
	// Verbs: %[1]s = brand.Tagline (banner), %[2]s = brand.Name (command),
	// %[3]s = brand.DisplayName (product name in prose). Routing every visible
	// mention through brand keeps `kez help` from drifting back to "zero".
...
Commands:
  ...
  models     List %[3]s model registry entries
  ...
  search     Search persisted local %[3]s session events
  find       Alias for search
  sessions   Inspect local %[3]s session lineage
  spec       Review and approve saved spec-mode drafts
  specialist Manage local %[3]s specialist profiles
  plugins    Inspect, install, and remove local %[3]s plugins
  backends   Inspect MCP, hook, and plugin backend lifecycle state
  skills     Inspect, install, and remove local %[3]s skills
  tools      Scaffold and list local %[3]s plugin-tools
  hooks      Inspect %[3]s hook configuration
  ...
  update     Check for %[3]s CLI updates
  ...
  serve      Run %[3]s protocol servers
...
`, brand.Tagline, brand.Name, brand.DisplayName)
	return err
}
...
func writePromptRequired(stderr io.Writer) int {
	if _, err := fmt.Fprintln(stderr, brand.LogPrefix+" Prompt required. Use `"+brand.Name+" exec \"prompt\"` or `"+brand.Name+" exec --file prompt.txt`."); err != nil {
		return 1
	}
	return 2
}

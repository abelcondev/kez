...
func TestMCPGateUntrustedExcludesProjectServer(t *testing.T) {
	...
	// This repo has no ./.kez/config.json, so there is no project MCP config to
...
// only a ./.kez/config.json that parses AND declares at least one server is true.
func TestProjectMCPConfigExists(t *testing.T) {
	writeCfg := func(t *testing.T, dir, body string) {
		t.Helper()
		if err := os.MkdirAll(filepath.Join(dir, ".kez"), 0o700); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(filepath.Join(dir, ".kez", "config.json"), []byte(body), 0o600); err != nil {
			t.Fatal(err)
		}
	}
...
// untrusted workspace actually declares project MCP servers in ./.kez/config.json,
...
// workspace whose only definition of a server lives in ./.kez/config.json, the gate

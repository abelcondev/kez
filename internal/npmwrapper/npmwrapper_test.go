...
func runWrapperFixtureWithEnv(t *testing.T, wrapperPath string, extraEnv []string, args ...string) (stdout string, stderr string, exitCode int) {
	...
	command.Env = append(withoutEnvKey(command.Env, "KEZ_LOCAL_CONTROL_HELPERS"), "KEZ_LOCAL_CONTROL_HELPERS=")
	command.Env = append(withoutEnvKey(command.Env, "KEZ_WRAPPER_SIMULATE_BUN"), extraEnv...)
...
func TestPostinstallComputesAssetPlan(t *testing.T) {
	...
	for _, tc := range cases {
		stdout, stderr, err := runPostinstall(t,
			"KEZ_INSTALL_DRY_RUN=1",
			"KEZ_INSTALL_PLATFORM="+tc.platform,
			"KEZ_INSTALL_ARCH="+tc.arch,
		)
		if err != nil {
		...
		wantURL := "https://github.com/abelcondev/kez/releases/download/v" + version + "/" + tc.wantAsset
...
func TestPostinstallSkipsUnsupportedPlatform(t *testing.T) {
	stdout, stderr, err := runPostinstall(t,
		"KEZ_INSTALL_DRY_RUN=1",
		"KEZ_INSTALL_PLATFORM=plan9",
		"KEZ_INSTALL_ARCH=x64",
	)
	if err != nil {
...
func TestPostinstallSkipsWindowsArm64(t *testing.T) {
	...
	stdout, stderr, err := runPostinstall(t,
		"KEZ_INSTALL_DRY_RUN=1",
		"KEZ_INSTALL_PLATFORM=win32",
		"KEZ_INSTALL_ARCH=arm64",
	)
	if err != nil {
...
func TestPostinstallHonorsSkipEnv(t *testing.T) {
	stdout, stderr, err := runPostinstall(t, "KEZ_SKIP_DOWNLOAD=1")
	if err != nil {
		t.Fatalf("KEZ_SKIP_DOWNLOAD should exit 0, got err=%v stderr=%s", err, stderr)
	}
	if strings.TrimSpace(stdout) != "" {
...
func TestNodeWrapperRunsFirstRunDownloaderWhenBinaryMissing(t *testing.T) {
	...
	// executes it; KEZ_SKIP_DOWNLOAD keeps the test offline, so the download
	...
	defer cancel()
	command := nodeWrapperCommand(ctx, node, wrapperPath, "--version")
	command.Env = append(command.Env, "KEZ_SKIP_DOWNLOAD=1")
	output, err := command.CombinedOutput()
	if ctx.Err() != nil {
...
func TestNodeWrapperGenericReportsBunRecoveryWhenInstalledByBun(t *testing.T) {
	wrapperPath := copyWrapperFixture(t)
	_, stderr, exitCode := runWrapperFixtureWithEnv(t, wrapperPath, []string{"KEZ_WRAPPER_SIMULATE_BUN=1"}, "--version")
	if exitCode != wrapperExitDoctor {
...
func TestNodeWrapperDoctorReportsBunRecoveryWhenInstalledByBun(t *testing.T) {
	wrapperPath := copyWrapperFixture(t)
	stdout, stderr, exitCode := runWrapperFixtureWithEnv(t, wrapperPath, []string{"KEZ_WRAPPER_SIMULATE_BUN=1"}, "doctor")
	if exitCode != wrapperExitDoctor {
...
func TestNodeWrapperDoctorAndGenericShareBunRecoveryCopy(t *testing.T) {
	wrapperPath := copyWrapperFixture(t)
	simulateBun := []string{"KEZ_WRAPPER_SIMULATE_BUN=1"}
...
func TestNodeWrapperClearsInheritedLocalControlHelperManifestWhenNoHelpers(t *testing.T) {
	...
	root := filepath.Dir(filepath.Dir(wrapperPath))
	nativePath := filepath.Join(root, "zero")
	if err := os.WriteFile(nativePath, []byte("#!/usr/bin/env sh\nif [ -n \"${KEZ_LOCAL_CONTROL_HELPERS+x}\" ]; then printf 'set\\n'; else printf 'unset\\n'; fi\n"), 0o755); err != nil {
		t.Fatalf("WriteFile native fixture: %v", err)
	}
	...
	defer cancel()
	command := nodeWrapperCommand(ctx, node, wrapperPath, "--version")
	command.Env = append(withoutEnvKey(command.Env, "KEZ_LOCAL_CONTROL_HELPERS"), `KEZ_LOCAL_CONTROL_HELPERS={"version":1,"helpers":{"agent-browser":{"command":"stale"}}}`)
	output, err := command.CombinedOutput()
	if ctx.Err() != nil {
		t.Fatalf("wrapper timed out launching native binary: %v; output: %s", ctx.Err(), output)
	}
	...
	if got := strings.TrimSpace(string(output)); got != "unset" {
		t.Fatalf("KEZ_LOCAL_CONTROL_HELPERS state = %q, want unset", got)
	}
}

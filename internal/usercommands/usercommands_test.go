...
func TestLoadParsesFrontmatterAndBody(t *testing.T) {
	root := t.TempDir()
	dir := filepath.Join(root, ".kez", "commands")
	...
	if !c.Project {
		t.Fatal("expected Project=true for a .kez/commands file")
	}
}
...
func TestLoadProjectOverridesUser(t *testing.T) {
	root := t.TempDir()
	userCfg := t.TempDir()
	writeFile(t, filepath.Join(userCfg, "kez", "commands"), "review.md", "USER version")
	writeFile(t, filepath.Join(root, ".kez", "commands"), "review.md", "PROJECT version")

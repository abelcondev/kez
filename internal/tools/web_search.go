...
import (
	...
	zeroSandbox "github.com/abelcondev/kez/internal/sandbox"
)
...
func (tool webSearchTool) Run(ctx context.Context, args map[string]any) Result {
	...
	if tool.backend == nil {
		return errorResult("Error: no search backend configured. Set KEZ_WEBSEARCH_BASE_URL (and KEZ_WEBSEARCH_API_KEY) to enable web_search.")
	}
...
// KEZ_WEBSEARCH_BASE_URL is unset (the tool then reports it as unconfigured).
func defaultSearchBackend() searchBackend {
	baseURL := strings.TrimSpace(os.Getenv("KEZ_WEBSEARCH_BASE_URL"))
	if baseURL == "" {
		return nil
	}
	return &httpSearchBackend{
		...
		baseURL:  baseURL,
		apiKey:   strings.TrimSpace(os.Getenv("KEZ_WEBSEARCH_API_KEY")),
		provider: strings.TrimSpace(os.Getenv("KEZ_WEBSEARCH_PROVIDER")),
	}
}
...
func (backend *httpSearchBackend) Search(ctx context.Context, query string, limit int) ([]searchResult, error) {
	...
	// query; without this the KEZ_WEBSEARCH_PROVIDER knob would be inert.
	if backend.provider != "" {

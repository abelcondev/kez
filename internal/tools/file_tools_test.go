...
func TestGrepSkipsAlwaysExcludedDirectories(t *testing.T) {
	...
	mustWrite(".kez/state.json", "needle here")
	...
	for _, forbidden := range []string{".git", ".kez", "node_modules", "vendor", ".worktrees"} {
...
func TestGlobSkipsWorkspaceExcludedDirectoriesAndBinaryFiles(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(root, ".kez", "state.json"), "{}")
	...
	for _, forbidden := range []string{".kez", "vendor", ".worktrees", "image.png"} {
		if strings.Contains(res.Output, forbidden) {

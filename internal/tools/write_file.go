...
func (tool writeFileTool) RunWithOptions(ctx context.Context, args map[string]any, options RunOptions) Result {
	...
	// Compiled branch guard: in a repo that requires PRs, refuse code writes on a
	...
	// Compiled quality gate: block a write that would leave a source file over
	// the line cap. Runs before any directory or file is created.
	if gateErr := qualitygate.Check(relativePath, priorContent, content); gateErr != nil {

...
import (
	...
	"github.com/abelcondev/kez/internal/qualitygate"
)
...
func (tool editFileTool) RunWithOptions(ctx context.Context, args map[string]any, options RunOptions) Result {
	...
	// Compiled branch guard: in a repo that requires PRs, refuse code edits on a
	// protected branch so feature work starts on a feature branch, not at push.
	if guardErr := branchguard.Check(tool.workspaceRoot, relativePath); guardErr != nil {
		return errorResult(guardErr.Error())
	}
	// Compiled quality gate: block an edit that would leave a source file over
	// the line cap (unless the file is already over and the edit does not grow it).
	if gateErr := qualitygate.Check(relativePath, content, updated); gateErr != nil {
	...
	// Optional format-on-write (KEZ_FORMAT_ON_WRITE). Must run BEFORE the

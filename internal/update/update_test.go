...
func TestCheckFallsBackReleaseURL(t *testing.T) {
	result, err := Check(context.Background(), Options{
		CurrentVersion: "0.1.0",
		// No explicit Repository: fall back to DefaultRepository (brand.Repository),
		// so the release URL points at the canonical kez repo.
		GOOS:   "linux",
		GOARCH: "amd64",
		Fetch: func(context.Context, string) (Release, error) {
	...
	wantURL := "https://github.com/" + brand.Repository + "/releases/tag/v0.2.0"
...
func TestCheckResolvesEndpointPrecedence(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.clearEnv {
				t.Setenv("KEZ_UPDATE_RELEASE_URL", "")
			} else {
				t.Setenv("KEZ_UPDATE_RELEASE_URL", tt.env)
			}
			options := tt.options
...
func releaseForTarget(t *testing.T, tag string, goos string, goarch string) Release {
	...
	return Release{
		TagName: tag,
		HTMLURL: "https://github.com/abelcondev/kez/releases/tag/" + tag,
		Assets: []Asset{

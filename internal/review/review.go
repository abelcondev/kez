...
var defaultCheckSpecs = []checkSpec{
	{Env: "KEZ_REVIEW_DIFF_CHECK", Label: "Diff hygiene", Command: "git diff --check"},
	{Env: "KEZ_REVIEW_TEST", Label: "Tests", Command: "go test ./..."},
	{Env: "KEZ_REVIEW_BUILD", Label: "Build", Command: "go run ./cmd/zero-release build"},
	{Env: "KEZ_REVIEW_SMOKE", Label: "Smoke build", Command: "go run ./cmd/zero-release smoke"},
}
...
func BuildSummaryInputFromEnv(env map[string]string) SummaryInput {
	return SummaryInput{
		...
		HeadSHA:      firstNonEmpty(env["KEZ_REVIEW_HEAD_SHA"], env["GITHUB_SHA"]),
		Checks:       BuildChecksFromEnv(env),
		ChangedFiles: ParseChangedFiles(env["KEZ_CHANGED_FILES"]),
	}
}
...
func parsePRNumber(env map[string]string) int {
	value := firstNonEmpty(env["KEZ_PR_NUMBER"], strings.Split(env["GITHUB_REF_NAME"], "/")[0])

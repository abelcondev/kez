...
func TestSaveDraftWritesPlainMarkdownWithCollisionSuffix(t *testing.T) {
	...
	if first.RelativePath != ".kez/specs/2026-06-08-oauth-redirect.md" {
...
func TestSaveDraftContainsAdversarialTitles(t *testing.T) {
	...
	for _, title := range []string{
		...
		t.Run(title, func(t *testing.T) {
			...
			if filepath.IsAbs(saved.RelativePath) || !strings.HasPrefix(saved.RelativePath, ".kez/specs/") {
...
func TestSubmitToolSavesSpecAndReturnsReviewControl(t *testing.T) {
	...
	if len(result.ChangedFiles) != 1 || result.ChangedFiles[0] != ".kez/specs/2026-06-08-implementation-plan.md" {
...
func TestLoadSpecFileRejectsPathsOutsideSpecDirectory(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected LoadSpecFile to reject a path outside .kez/specs")
	}
}

func TestLoadSpecFileRejectsNonRegularFiles(t *testing.T) {
	root := t.TempDir()
	dir := filepath.Join(root, filepath.FromSlash(".kez/specs/not-a-file.md"))

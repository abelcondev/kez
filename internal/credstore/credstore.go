...
// headless/CI machines, so it is opt-in via KEZ_CRED_STORAGE=keyring). A plaintext
// file is available only as an explicit KEZ_CRED_STORAGE=file opt-out. OAuth tokens
...
import (
	...
	"github.com/abelcondev/kez/internal/keyring"
	"github.com/abelcondev/kez/internal/securefile"
)
...
type Options struct {
	...
	// (plaintext opt-out). When empty it falls back to KEZ_CRED_STORAGE, then auto:
	...
	// users can still opt into the keyring with KEZ_CRED_STORAGE=keyring.
	Storage string
...
func New(options Options) (*Store, error) {
	...
	if storage == "" {
		storage = strings.TrimSpace(env("KEZ_CRED_STORAGE"))
	}
	if storage == "" {
		...
		// encrypted file; keyring stays available via an explicit KEZ_CRED_STORAGE.

...
// explicit KEZ_SKILLS_DIR override wins; otherwise it is
...
func DefaultDir(env map[string]string) string {
	if override := strings.TrimSpace(envValue(env, "KEZ_SKILLS_DIR")); override != "" {
		return override
	}
	...
	return filepath.Join(base, "kez", "skills")
}
...
// os.UserHomeDir(). KEZ_SKILLS_DIR is intentionally ignored — agents is a

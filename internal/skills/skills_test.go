...
func TestDefaultDirHonorsEnvOverride(t *testing.T) {
	got := DefaultDir(map[string]string{"KEZ_SKILLS_DIR": "/custom/skills"})
...
func TestDefaultDirHonorsXDGDataHome(t *testing.T) {
	...
	want := filepath.Join("/xdg/data", "kez", "skills")
	if got != want {
...
func TestDefaultDirFallsBackToHome(t *testing.T) {
	...
	want := filepath.Join("/home/zero", ".local", "share", "kez", "skills")
	if got != want {
		t.Fatalf("DefaultDir = %q, want %q", got, want)
	}
}
...
func TestAgentsDirIgnoresZeroSkillsDir(t *testing.T) {
	...
	if got != agents {
		t.Fatalf("AgentsDir with KEZ_SKILLS_DIR set = %q, want %q", got, agents)
	}
}
